...
class Example14 {
  val init_img = Fill[Color, Id](ColorVector.hsla(Angle(Pi), .8, .6))
  val r = .1
    
  val color_fun = (angle: Angle, color: Color) => {
    color.addH(angle).toColor()
  }
...
//  val conic = Conic[Color, Id](Point(.5, .5), r, color_fun)
  ...
  drawToFileOld[Color, Id](fun(init_img), trivialColorFun, "example14_3.png", 0, 1, 0, 1, 1000, 1000)
}
