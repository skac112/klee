...
class Example4 {
  ...
  val ring1 = Ring[Color, Color, Id](c + Point(.1, .1), .2, .25, Color.blue(.7))
  val ring2 = Ring[Color, Color, Id](c - Point(.1, .1), .2, .25, Color.red(.7))
  val ring3 = Ring[Color, Color, Id](c - Point(.1, .1), .3, .35, Color.green(.7))
  val comp = Composition(List[ImgTrans[Color, Color, Id]](ring2, ring1, ring3, bh1, bh2))
  drawToFile[Color, Id](comp(init_img), trivialColorFun, "example4_3.png", 0, 1, 0, 1, 1500, 1500)
