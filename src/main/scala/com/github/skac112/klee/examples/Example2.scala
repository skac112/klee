...
class Example2 {
  val motion_eq = Rotation(.25 * Pi)
  val dyn_sys = MotionEqDynamicalSystem(motion_eq, .001)
  val fun = DynSysDisplacer(dyn_sys, 1.0)
  val src_img = Lines(0, 0, 20, 20, 1, Color.black, Color.white)
  val dst_img = fun(src_img)
  drawToFile(dst_img, "example2.png", -500, 499.0, -500, 499.0, 1000, 1000)
}
