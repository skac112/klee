...
import com.github.skac112.klee.{Composition, blendColors, drawToFile, trivialColorFun}
import com.github.skac112.vgutils.{Angle, Color, Point}

import scala.math._

class Example15 {
  val init_img = Fill[Color, Id](Color.hsla(Angle(Pi), .8, .2))
  val a = 10.0
  val r = 12.0/a
  val omega_f = 0
...
//    val ch_col = color.addH(angle * 3)
    ...
    blendColors(ch_col, color, strength)
  }
...
//  val fun = Composition[Color, Id](conics)
  drawToFile[Color, Id](fun(init_img), trivialColorFun, "example15_9.png", 0, 1, 0, 1, 1000, 1000)
}
