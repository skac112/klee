...
/**
  ...
  */
abstract class Colorizer[I,  M[_]] extends LocalImgTrans[I, M] {
  override def area(implicit m: Monad[M]): ImgArea = WholeArea()
  def colorFun: (I) => M[I]
  def applyToAir: Boolean = true

  override def applyInArea(img: Img[I, M], ip: ImgPoint[I, M])(implicit m: Monad[M]) = if (applyToAir || ip.land) {
    InstantImgPoint(ip.point, newColorM(img, ip.point), ip.land)
  } else {
    ip
  }

  protected def newColorM(img: Img[I, M], ptM: M[Point])(implicit m: Monad[M]): M[I] = for {
    pt <- ptM
    color <- img(pt)
